...
class RestaurantResource(Resource):
    ...
    def get(self):
        if 0==1: #redis_cache.exists(RESTAURANT_LIST):
    ...
    def post(self):
        ...
        try:
            restaurant_schema.load(json_data)
        except ValidationError as err:
            return {"status": "error", "data": err['messages']}, 422
        restaurant = RestaurantModel.query.filter_by(name=json_data['name']).first()
        if restaurant:
...
class RestaurantItemResource(Resource):
    ...
    def get(self, id):
        ...
        restaurant = restaurants_schema.dump(restaurant)
        return {'status': 'success', 'data': restaurant}, 200


    def delete(self, id):
        restaurant = RestaurantModel.query.filter_by(id=id).delete()
        db.session.commit()
        result = restaurant_schema.dump(restaurant)
